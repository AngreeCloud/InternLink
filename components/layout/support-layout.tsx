      ...
      <div className={`transition-all duration-300 ${collapsed ? "lg:pl-[72px]" : "lg:pl-72"}`}>
        <header className="flex h-14 shrink-0 items-center justify-between border-b border-border px-4">
          <div className="flex items-center gap-2">
            ...
            <Link href="/support" className="flex items-center gap-2 font-semibold"><Headset className="h-5 w-5 text-primary" /><span>Support</span></Link>
          </div>
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56">
              <DropdownMenuLabel><p className="text-sm font-medium">{user.displayName}</p><p className="text-xs text-muted-foreground">{user.email}</p></DropdownMenuLabel>
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={handleLogout} disabled={loggingOut}><LogOut className="mr-2 h-4 w-4" />{loggingOut ? "A sair..." : "Logout"}</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </header>
